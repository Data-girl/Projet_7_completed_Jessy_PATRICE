...
st.sidebar.header("Application de prêt automatique")
...
option = st.selectbox(
    ...
    (
        ...
        "Afficher les graphiques de l'importance globale",
    ),
)
        ...
        if client_id_input == "" or client_id_input.isalpha():
            ...
            if pred_str_format == "Prêt accordé, client sûr":
                ...
                fig = px.bar(
                    ...
                    color_continuous_scale=["red", "green"], range_color=[-0.01,0])
...
if option == "Afficher les graphiques de l'importance globale":
    ...
    fig = px.bar(plot_importance, x="Feature", y="Weight",color = "Weight", 
                            color_continuous_scale=["red", "green"], range_color=[-0.5,0.002])
    st.plotly_chart(fig, use_container_width=True)
